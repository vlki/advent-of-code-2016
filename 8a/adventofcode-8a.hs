...
countLitPixels screen = length $ filter (\x -> x) $ toList screen
...
applyCommandsAndCountLitPixels :: [String] -> Int
applyCommandsAndCountLitPixels commands = countLitPixels $ applyCommands commands

main = do
    input <- getContents
    putStrLn $ show $ applyCommandsAndCountLitPixels $ lines input
